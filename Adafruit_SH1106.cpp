...
void Adafruit_SH1106::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  initHwr(reset);
  initDisplay(vccstate, i2caddr);
} 

void Adafruit_SH1106::initDisplay( uint8_t vccstate, uint8_t i2caddr) {
    _vccstate = vccstate;
  _i2caddr = i2caddr;
...
void Adafruit_SH1106::initHwr( bool reset ) {
  // set pin directions
  if (sid != -1){
    pinMode(dc, OUTPUT);
    pinMode(cs, OUTPUT);
